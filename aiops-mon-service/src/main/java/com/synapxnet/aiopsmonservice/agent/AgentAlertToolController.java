 ...
 * English: Exposes native alert evidence through the scoped Agent contract.
 */
@RestController
public class AgentAlertToolController {
     ...
     * English: Injects native alert lookup, evidence assembly and scoped resource authorization.
     */
    public AgentAlertToolController(
            AlertHistoryService alertHistoryService,
            AlertEvidenceAssembler evidenceAssembler, OperationsReadAccess access) {
        this.access = access;
    ...
    public AgentContract.ToolResponse<AlertEvidence> invoke(
        ...
        access.requireAgentResource(context.workspaceId(), "alert", arguments.alertUid());
        AlertEvidence evidence = read(arguments.alertUid());
        return AgentContract.success(evidence, context, "XnetAIops/mon", evidence.sourceRecordVersion(), startedNanos);
    }

    /** 读取实际告警，资源不存在时不返回演练数据。 English: Reads a native alert and never substitutes synthetic data for missing records. */
    public AlertEvidence read(String uid) {
        return evidenceAssembler.assemble(loadAlert(new AlertGetArguments(null, uid)));
    }
     ...
     * @return 告警领域记录
     * English: Looks up exactly one native alert by its stable identifier; missing records are not replaced.
     */
    ...
    /** 表示告警工具的互斥查询参数。 English: Defines a typed, read-only native evidence contract and its source metadata. */
    public record AlertGetArguments(String alertId, String alertUid) {
    }

    /** 表示告警关联的受控资源引用。 English: Defines a typed, read-only native evidence contract and its source metadata. */
    public record RelatedResource(String type, String uid, String namespace, String name) {
    }

    /** 表示可跨平台引用的告警证据。 English: Defines a typed, read-only native evidence contract and its source metadata. */
